...
class TestAPIClientTransport(unittest.TestCase):
    ...
    def test_transport_clone_reuses_http_transport_with_a_new_prefix(self) -> None:
        ...
        self.assertEqual(
            http_transport.requests[0].url,
            "https://example.atlassian.net/wiki/api/v3",
        )
    ...
    def test_encodes_a_json_request_with_basic_authentication(self) -> None:
        ...
        client.make_json_request(
            "PUT",
            "/pages/A page",
            parameters={"expand": "body.atlas_doc_format", "title": "A page"},
        ...
        self.assertEqual(
            request.url,
            "https://example.atlassian.net/wiki/api/v2/pages/A%20page?"
            "expand=body.atlas_doc_format&title=A+page",
        )
    ...
    def test_collects_paginated_results_from_host_relative_links(self) -> None:
        transport = MockTransport(
            [
                ...
                MockResponse.from_json({"results": [{"id": "2"}]}),
            ]
        )
    ...
    def test_normalizes_http_errors(self) -> None:
        ...
        for status, error_type in cases:
            with self.subTest(status=status):
                ...
                client = APIClient(
                    "example.atlassian.net", "user", "token", transport=transport
                )
